/**
 * Tara AI — Automated Evaluation Suite (Step 5)
 *
 * Run:  npm run eval          (requires server running on localhost:3000)
 *
 * Sends 12 natural-language finance questions to POST /ask, validates each
 * response against an expectedSnippet (case-insensitive substring match),
 * and prints a clean [PASS] / [FAIL] report with a summary table.
 */
...
interface EvalCase {
  ...
  scenario: string;
  question: string;
  /** Case-insensitive substring(s) that MUST appear in the answer. */
  expectedSnippets: string[];
  /** If true, ALL snippets must match. If false (default), ANY one is enough. */
  matchAll?: boolean;
}
...
const TEST_CASES: EvalCase[] = [
  // ── 1. Single lookup + specific date boundary ──
  {
    id: 1,
  ...
  // ── 2. Net spend / refund subtraction ──
  {
    id: 2,
  ...
  // ── 7. No-data validation (date range before dataset) ──
  {
    id: 7,
  ...
  // ── 9. Portfolio Realised Return (Path B — single holding) ──
  {
    id: 9,
  ...
  // ── 11. Specific merchant variant — Amazon aliases ──
  {
    id: 11,
  ...
  // ── 12. March 2025 date boundary (latest month in dataset) ──
  {
    id: 12,
